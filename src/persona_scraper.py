...
class PersonaScraper:
    """Scrapes and extracts persona data from OpenAI exports"""
    ...
    def load_and_scrape(self) -> Dict[str, Any]:
        """Load export file and scrape all persona data"""
    ...
    def export_to_json(self, output_path: str):
        ...
        print(f"✓ Persona data exported to: {output_file}")
        return output_file
    
    def print_summary(self):
        """Print summary of scraped data"""
        print("\n" + "="*60)
        print("PERSONA DATA EXTRACTION SUMMARY")
        print("="*60)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python persona_scraper.py ./export_directory/ my_persona.json")
        sys.exit(1)
    
    export_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else "persona_data.json"
    
    try:
        print(f"🔍 Scraping persona data from: {export_path}")
        scraper = PersonaScraper(export_path)
        scraper.load_and_scrape()
        scraper.print_summary()
        scraper.export_to_json(output_path)
        print(f"\n✅ Success! Your persona data is now sovereign and free! 🎉")
